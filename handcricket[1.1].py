...
Rules:
      ...
      Same for the computer innigs too
...
def yourinnings(format,state,computer_score=0):
        ...
        if ( format == 1 or format == 2):
            ...
            while mainflag:
                ...
                if state==1:
                    if overs>=5:
                        if format==1 and wickets!=10:
                            print(f"Projected Score: {round(run_rate*20)}")
                        elif format==2 and wickets!=10:
                            print(f"Projected Score: {round(run_rate*50)}")
                elif state==2:
                    if format==1 and computer_score>=score and wickets!=10:
                        print(f"You require {computer_score-score+1} runs from {120-balls-(overs*6)} balls to win. Required Run Rate: {round(((computer_score-score+1)*6)/(((20-overs)*6+(6-balls))),2)}")
                    elif format==2 and computer_score>=score and wickets!=10:
                        print(f"You require {computer_score-score+1} runs from {300-balls-(overs*6)} balls to win. Required Run Rate: {round(((computer_score-score+1)*6)/(((50-overs)*6+(6-balls))),2)}")
        ...
        if ( format == 1 or format == 2):
            ...
            while mainflag:
                ...
                if ( balls==6):
                    ...
                    balls=0
                if state==2:
                    required_rate1=round(((user_score-score+1)*6)/(((20-overs)*6+(6-balls))),2)
                    required_rate2=round(((user_score-score+1)*6)/(((50-overs)*6+(6-balls))),2)
                    if format==1:
                ...
                if state==1:
                    ...
                    if format==1 and user_score>=score and wickets!=10:
                        print(f"Computer requires {user_score-score+1} runs from {120-balls-(overs*6)} balls to win. Required Run Rate: {round(((user_score-score+1)*6)/(((20-overs)*6+(6-balls))),2)}")
                    elif format==2 and user_score>=score and wickets!=10:
                        print(f"Computer requires {user_score-score+1} runs from {300-balls-(overs*6)} balls to win. Required Run Rate: {round(((user_score-score+1)*6)/(((50-overs)*6+(6-balls))),2)}")
def cricketgame():
    format=formatchoice()
    state1=toss()
    if state1==1:
